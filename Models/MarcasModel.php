...
class MarcasModel extends Query{
    ...
    public function registrarMarca(string $nombre, string $nombre_corto){
        $this->nombre = $nombre;
        $this->nombre_corto = $nombre_corto;
        $verificar = "SELECT * FROM marcas WHERE nombre = '$this->nombre'";
        $existe = $this->select($verificar);
        if (empty($existe)) {
            $sql = "INSERT INTO marcas(nombre, nombre_corto) VALUES (?,?)";
    ...
    public function modificarMarca(string $nombre, string $nombre_corto, int $id){
        $this->nombre = $nombre;
        $this->nombre_corto = $nombre_corto;
        $this->id = $id;
        $sql = "UPDATE marcas SET nombre = ?, nombre_corto = ? WHERE id = ?";
